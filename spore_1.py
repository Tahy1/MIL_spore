...
def main():
    ...
    model.load_state_dict(torch.load('r18-pretrained.pth'))
...
#    optimizer = optim.SGD(model.parameters(), lr=1e-3, momentum=0.9, weight_decay=1e-4)
    ...
    train_dset = MILdataset('train-100.lib',transform=trans)
    train_loader = torch.utils.data.DataLoader(
        train_dset,
        batch_size=128,shuffle=False,
        pin_memory=False)
    test_dset = MILdataset('test-100.lib',transform=trans)
...
#        torch.save(maxs, 'maxs/%d.pth'%epoch)
        sf(topk)
        ## ③准备训练集
